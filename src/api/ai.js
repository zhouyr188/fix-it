...
function isChineseQuestion(q) {
  ...
  if (q.trim().endsWith('？') || q.trim().endsWith('?')) return false; // 问句当场退货——翻译的是陈述不是提问
  if (/请(分析|谈谈|描述|说明|比较|评价|阐述)/.test(q)) return false; // 指令句退货——这是作文题不是翻译题
  return chinese >= 4 && chinese > letters;
}
...
export function buildQuestionPrompt(trapType, level) {
  ...
  '5. 题面应包含值得斟酌的表达（如因果关系、程度对比、多动作衔接），让翻译有思考空间，而不仅仅是简单罗列事实；\n' +
  ...
  '3. 难度应高于中等档：句子更长、结构更复合（从句嵌套、抽象名词、逻辑衔接词），但仍然是一个可翻译的陈述句，绝不是写作任务；\n' +
  ...
  return [
    { role: 'system', 
      ...
      '时态：通过自然的时间信息考察英文时态。' + 
      '词形：通过自然语境考察动词形式、名词单复数、词性等变化。' + 
      '直译：通过自然中文表达考察英语中的自然表达、搭配、词语选择或句式转换。' + 
      '参考译文必须准确、自然，并符合指定难度。它只是合理的参考表达，不代表唯一答案。' + 
      '难度主要通过话题、思维要求、词汇和句法自然体现，不要使用生僻知识或刻意堆砌复杂结构。' + 
